...
Purpose: To create some basic tools for loading 3D meshes
from off files and for randomly sampling points on the
surface by area
"""
...
def sample_by_area(VPos, ITris, n_points, colPoints=False):
    """
    ...
    Returns
    -------
    (Ps : NDArray (n_points, 3) array of sampled points, 
     Ns : Ndarray (n_points, 3) of normals at those points   )
    """
    ...
    samples = np.sort(np.random.rand(n_points))
    ...
    tidx = np.zeros(n_points, dtype=np.int64)
    idx = 0
    for i in range(len(FSamples)):
        tidx[idx:idx+FSamples[i]] = i
        idx += FSamples[i]
    N = np.zeros((n_points, 3)) #Allocate space for normals
    idx = 0
    ...
    idxreg = np.arange(n_points, dtype=np.int64)
...
def get_d2_feature(X, n_dists, bins):
    """
    Parameters
    ----------
    X: ndarray(N, 3)
        3D Point cloud
    n_dists: int
        Number of pairwise distances to sample
    bins: int
        Number of points to use on the histogram from [0, 2]
    """
    ## Step 1: RMS Normalize
    X = X - np.mean(X, axis=0)[None, :]
    X = X*np.sqrt(X.shape[0]/np.sum(X**2))
    ## Step 2: Compute distance between random pairs of points
    I = np.random.randint(0, X.shape[0], n_dists)
    J = np.random.randint(0, X.shape[0], n_dists)
    d = np.sqrt(np.sum((X[I, :] - X[J, :])**2, axis=1))
    return np.histogram(d, bins=bins, range=(0, 2))[0]
